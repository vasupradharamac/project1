...
root.iconbitmap(r"D:/pipey/code_docs.ico")
...
def print_file(e):
	...
	#Grab FileName
	print_to_file = filedialog.askopenfilename(initialdir="C:/gui/", title="Open File", filetypes=(("Text Files", "*.txt"),("HTML Files", "*.html"),("Python Files", "*.py"), ("All Files","*.*")))
	if print_to_file:
...
#text box
my_text =  Text(my_frame, width=97, height=25, font=("Lato",16), selectbackground="#ADD8E6", selectforeground="black", undo=True, yscrollcommand=text_scroll.set, wrap="none", xscrollcommand=hor_scroll.set)
my_text.pack()
...
edit_menu.add_command(label="Find & Replace", accelerator="(Ctrl+F)", command=lambda: find_text(False))
edit_menu.add_separator()
...
style_menu.add_command(label="All Text", command=lambda: all_text_color(False), accelerator="(Ctrl+A)")
style_menu.add_separator()
style_menu.add_command(label="Text Color", command=text_color)
